...
interface Tither {
  ...
  parishioner?: { id: number; name: string; phone: string | null };
  joined_at: string;
  periodicity: string;
...
export default function Tithers() {
    ...
    try {
      const payload = {
        ...
        suggested_amount: form.suggested_amount
          ? parseFloat(form.suggested_amount)
          : null,
  ...
  const handleDelete = async () => {
    ...
    setSaving(true);
    try {
      await api.delete(`/parish-admin/tithers/${selected.id}`);
      setDeleteOpen(false);
      setSelected(null);
      fetchData();
    } catch (err) {
  ...
  const columns = [
    {
      ...
      label: 'Nº',
      render: (t: Tither) => (
        <span className="text-xs font-mono text-gray-500">#{t.tither_number}</span>
      ),
    },
    {
      key: 'parishioner',
  ...
  return (
    <div>
      ...
      <Modal
        ...
        title={selected ? 'Editar Dizimista' : 'Novo Dizimista'}
        size="lg"
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setForm({ ...form, suggested_amount: e.target.value })
              }
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none text-sm"
            />
          </div>
      ...
      <ConfirmDialog
        ...
        message={`Excluir "${selected?.parishioner?.name}"? Esta ação não pode ser desfeita.`}
